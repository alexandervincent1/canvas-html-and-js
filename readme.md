Optimization:

<!-- generatesine should be done when clicked currently its being done when the application starts running which means. that if you were to change the values and wanted generatesine original you would just get the same not a true sine signal. -->
<!-- how long the line in the graph should be? 1024 to much 32 to little -->

To add:

<!-- options eg, input from user, wav? keyboard? picture? -->
<!-- another graph for amplitude or magnitude? (dont quite remember what it was called) -->
<!-- frontend, a clean looking dashboard thats not to confusing (maybe in japanese?) -->
<!-- maybe theres another graph, calculation that could be displayed to be able to explain it better? -->
<!-- import by file wav file. -->
<!-- The ability to select multiple points at once area of selection (rectangle tool) -->
<!-- addition, subtraction, division, multiply, sawtooth, squarewave all the basics for the own formula  -->
<!-- input how many points you want eg 1024, 64... -->
<!-- flip half of the graph to the left of it, button. -->
<!-- instead of a bunch of buttons add a multiple choice field -->
<!-- show how many waves de graph has inspo leonwurr.com -->
<!-- add points for the spikes on the fft graph -->

nyqvist formula?

adjust the freq of the signal only selectable from choosen methods

display the levels

En slider: “Levels: 1–6”
Ett input‑fält: “Enter levels”
En label som visar max levels baserat på N
t.ex. N=64 → max 6 levels 
När användaren ändrar slider → uppdatera grafen
Du kan färgkoda varje level
Du kan lägga dem i separata sektioner
Du kan låta användaren expandera/collapsa levels
zoom function on the second graph?
multiple graphs to display differnet levels? or just colors per level?

switch the graphs
dwt only display the first level and then transform it
bug fixing:

the dwt graph changes to dft when updating the points in the first graph
<!-- match the graph perfectly (look at the end of it) -->
<!-- the dragable bar disappears when a point shows up. -->

